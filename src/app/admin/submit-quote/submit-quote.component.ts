...
export class SubmitQuoteComponent implements OnInit {
  ...
  getPatientTests(){
    ...
    this.healthService.getCollections(obj).subscribe((data) => {
      ...
      console.log(this.patientsArray.value);
      this.fetchQuote();
    });
  }
  // 
  ...
  fetchQuote() {
   ...
   /*  for(let i = 0; i < this.patientsArray.value.length; i++) {
      this.patientsArray.value[i]['price'] = res[i].item;
    } */

    console.log(res);
    console.log(this.patientForm.controls.value);
    return res
  }
}
